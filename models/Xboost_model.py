...
class XGBoostModel:
	...
	def save_model(self, path="model_files/xgboost.pkl"):
		os.makedirs(os.path.dirname(path), exist_ok=True)
		joblib.dump(self.model, path)
		print(f"Model saved to: {path}")

	def save_metrics(self, metrics_df, path="results/xgboost_metrics.csv"):
		os.makedirs(os.path.dirname(path), exist_ok=True)
	...
	def run(self):
		...
		metrics_df = evaluate_model("XGBoost", self.y_test, y_pred, y_probs)
